import {
  Area, CartesianGrid, ComposedChart, Label, LabelList, Line, ReferenceLine,
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="chart-tip">
      ...
      {row.band && <div className="tip-range">expected {row.band[0]}–{row.band[1]}</div>}
    </div>
  )
}

// Patient-inflow chart: recent actuals + forecast line, an ~80% confidence band
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={rows} margin={{ top: 18, right: 20, bottom: 4, left: -8 }}>
        ...
        <Area dataKey="band" stroke="none" fill={BAND} fillOpacity={0.35} name="Expected range" isAnimationActive={false} connectNulls={false} />
        {avg != null && (
        ...
        <Line dataKey="actual" name="Actual" stroke={ACTUAL} strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
        <Line dataKey="predicted" name="Forecast" stroke={PREDICTED} strokeWidth={2}
